...
## Run 164
- Hypothesis: `[restart-no-ml]` The equal-weight signed z-score baseline may still be over-compressing glucose signal, so applying the same `log1p` treatment used for CRP to `SGP` could improve ranking while keeping the rule fully elementary.
- Change: `[restart-no-ml]` Added a `log1p` transform on `SGP` before train-split standardization and scoring, while leaving the signed clipped z-score sum otherwise unchanged.
- Result: `val_cindex` **0.747037** vs kept no-ML baseline **0.743580**.
- Decision: **keep**; updated `last_kept_train.py` to match the new `train.py`.
- Learning: A second simple log transform helped materially on the restart baseline, suggesting at least one more right-skewed biomarker benefits from compression before z-scoring.
- Next: Stay in the same elementary family and test one additional single-feature transform or clipping idea from this stronger no-ML baseline.

## Run 165
- Hypothesis: `[restart-no-ml]` White blood cell count is an inflammatory burden marker with a right-skewed high-risk tail, so `log1p` compression may preserve direction while reducing outlier dominance in the equal-weight score.
- Change: `[restart-no-ml]` Added a `log1p` transform on `WCP` before train-split standardization, alongside the existing `SGP` and `CRP` transforms.
- Result: `val_cindex` **0.745462** vs kept no-ML baseline **0.747037**.
- Decision: **discard**; restored `train.py` from `last_kept_train.py`.
- Learning: The current baseline already captures enough inflammatory signal from `CRP` and raw `WCP`; compressing `WCP` removed useful rank information instead of stabilizing it.
- Next: Try a different skewed organ-dysfunction marker rather than another inflammation-only compression.

## Run 166
- Hypothesis: `[restart-no-ml]` Alkaline phosphatase often shows a long right tail in liver and cholestatic stress states, so `log1p` compression may improve robustness while keeping the same risk direction and equal-weight structure.
- Change: `[restart-no-ml]` Added a `log1p` transform on `APPSI` before train-split standardization, alongside the existing `SGP` and `CRP` transforms.
- Result: `val_cindex` **0.745337** vs kept no-ML baseline **0.747037**.
- Decision: **discard**; restored `train.py` from `last_kept_train.py`.
- Learning: Compressing `APPSI` weakened ranking slightly, so the kept baseline seems to benefit from preserving more raw spread in this marker.
- Next: Try a different high-skew organ marker rather than another hepatobiliary-tail compression.

## Run 167
- Hypothesis: `[restart-no-ml]` Creatinine is a renal dysfunction marker whose clinical effect often feels more multiplicative than linear, so `log1p` compression may preserve ordering while reducing leverage from a small number of very high values.
- Change: `[restart-no-ml]` Added a `log1p` transform on `CEP` before train-split standardization, alongside the existing `SGP` and `CRP` transforms.
- Result: `val_cindex` **0.749897** vs kept no-ML baseline **0.747037**.
- Decision: **keep**; updated `last_kept_train.py` to match the new `train.py`.
- Learning: The new baseline benefits from compressing a third right-skewed marker, and the helpful family now looks more like broad tail control for metabolic, inflammatory, and renal stress rather than a CRP-only special case.
- Next: Continue from this stronger baseline and test one more medically motivated single-change idea.

## Run 168
- Hypothesis: `[restart-no-ml]` Chronic inflammation may matter more when both CRP and white count are elevated together, so giving the CRP/WCP pair one additional shared vote could sharpen risk ranking.
- Change: `[restart-no-ml]` Added one extra inflammatory burden term equal to the average of signed `CRP` and `WCP` z-scores on top of the current signed z-score sum.
- Result: `val_cindex` **0.738447** vs kept no-ML baseline **0.749897**.
- Decision: **discard**; restored `train.py` from `last_kept_train.py`.
- Learning: Double-counting inflammation was too blunt; the equal-weight base score already carries enough inflammatory information, and a direct CRP/WCP boost overwhelms more balanced multisystem signal.
- Next: Try a different clinical syndrome term that mixes inflammation with a complementary domain instead of reweighting inflammation alone.

## Run 169
- Hypothesis: `[restart-no-ml]` Low albumin and high CRP together often mark inflammatory frailty or catabolic burden, so giving that pair one additional shared vote might surface a clinically coherent risk axis.
- Change: `[restart-no-ml]` Added one extra frailty-inflammation term equal to the average of signed `AMP` and `CRP` z-scores on top of the current signed z-score sum.
- Result: `val_cindex` **0.741884** vs kept no-ML baseline **0.749897**.
- Decision: **discard**; restored `train.py` from `last_kept_train.py`.
- Learning: This pairwise frailty boost was also too blunt; directly reweighting a clinically intuitive pair still hurts when the baseline already mixes multiple systems evenly.
- Next: Try a non-inflammatory syndrome term instead of another pair that includes CRP.

## Run 170
- Hypothesis: `[restart-no-ml]` Higher RDW and higher mean cell volume can jointly reflect dysmature erythropoiesis and physiologic stress, so giving that red-cell morphology pair one extra shared vote may capture hematologic aging burden not fully expressed by the base equal-weight sum.
- Change: `[restart-no-ml]` Added one extra hematologic term equal to the average of signed `MVPSI` and `RWP` z-scores on top of the current signed z-score sum.
- Result: `val_cindex` **0.753877** vs kept no-ML baseline **0.749897**.
- Decision: **keep**; updated `last_kept_train.py` to match the new `train.py`.
- Learning: A targeted hematologic composite helped where the inflammation-centric composites failed, which suggests the current rule benefits more from complementing existing signal with a distinct red-cell aging axis than from further reweighting CRP-centered biology.
- Next: Use the final iteration on one more clinically distinct syndrome term from this stronger baseline.

## Run 171
- Hypothesis: `[restart-no-ml]` High total white count plus low lymphocyte percentage can reflect immune stress with reduced reserve, so giving that pair one shared extra vote might add a clinically coherent immune-aging axis to the improved baseline.
- Change: `[restart-no-ml]` Added one extra immune-stress term equal to the average of signed `LMPPCNT` and `WCP` z-scores on top of the improved hematologic baseline.
- Result: `val_cindex` **0.744677** vs kept no-ML baseline **0.753877**.
- Decision: **discard**; restored `train.py` from `last_kept_train.py`.
- Learning: The extra immune-stress composite over-weighted one domain again; the useful additive composite so far is the hematologic pair, while further pairwise boosts keep degrading the balanced whole-body score.
- Next: Future restart-no-ML work should stay closer to the winning tail-compression-plus-hematology family rather than adding more pairwise syndrome votes.
