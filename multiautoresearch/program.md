...
```text
commit	development_cindex	baseline_cindex	delta_cindex	memory_gb	status	candidate_path	description
```

- `commit`: short git hash
- `development_cindex`: full-development C-index, or `0.000000` for crashes
- `baseline_cindex`: current kept-baseline C-index used for comparison
- `delta_cindex`: `development_cindex - baseline_cindex`
- `memory_gb`: peak memory in GB, or `0.0` for crashes
- `status`: `saved`, `keep`, `discard`, or `crash`
- `candidate_path`: saved snapshot path for `saved` and `keep` rows, blank otherwise
...
3. Run `python manage_kept.py restore` so `train.py` starts from `last_kept_train.py`.
